/**
 * Server-side slot evaluation (keep in sync with mobile `game-context` spin resolution).
 * Used by Edge Function `spin` only.
 */
...
export const BET_OPTIONS = [10, 25, 50, 100, 250, 500] as const
...
function checkPaylines(
  ...
  const gridSyms = gridIds.map((col) => col.map((id) => {
  ...
  const totalWin = lines.reduce((sum, line) => {
    ...
    const baseWin = Math.floor(bet * (sym.value / 10) * line.multiplier * jackpotMult)
    return sum + baseWin
  }, 0)
